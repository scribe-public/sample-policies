...
typeLibrary = "library"
pkgNpm = "npm"
bomv14Predicate = "https://cyclonedx.org/bom/v1.4"
bomV15Predicate = "https://cyclonedx.org/bom/v1.5"


checkBomStatement(statement) {
    evidence.checkIntoto(statement)
    evidence.checkPredicate(statement, bomv14Predicate)
}

checkBomStatement(statement) {
    evidence.checkIntoto(statement)
    evidence.checkPredicate(statement, bomV15Predicate)
}

getBom(statement) = bom {
    checkBomStatement(statement)
    predicate := evidence.getPredicate(statement)
    bom := predicate.bom
}

getBom(statement) = predicate {
    predicate := evidence.getPredicate(statement)
    not predicate.bom
}
...
getContentType(statement) = contentType {
    context := evidence.getEvContext(statement)
...
getBomFile(bom, filePath) = fileMap {    
    fileMap := { x["name"]: content |
        some i,j
        files := getComponentsByTypeAndGroup(bom, typeFile, typeFile)
        x := files[i]
        contains(x.name, filePath)

        prop := x.properties
        prop[j].name == "content"
        content := base64.decode(prop[j].value)
    }
}


getLastCommitUsers(bom) = users {    
