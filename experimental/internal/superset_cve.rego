...
raw_target_name = target_name {
	apply_target_filter_verifier
	target_name := clear_domain_name(verifier.input_name)
}

raw_target_name = target_name {
	apply_target_filter_args
	not apply_target_filter_verifier
	target_name := clear_domain_name(args.input_name)
}

## Get target tag
semver_regex := `v?([0-9]+)(\.[0-9]+)?(\.[0-9]+)?(-([0-9A-Za-z\-]+(\.[0-9A-Za-z\-]+)*))?(\+([0-9A-Za-z\-]+(\.[0-9A-Za-z\-]+)*))?`

find_semver_tag(tags) {
	some tag in tags
...
raw_target_tag = target_tag {
	apply_target_filter_verifier
	count(verifier.tag) > 0
	find_semver_tag(verifier.tag)

	some tag in verifier.tag
	regex.match(semver_regex, tag)
	target_tag := tag
}
...
raw_target_tag = target_tag {
	apply_target_filter_verifier
	count(verifier.tag) > 0
	not find_semver_tag(verifier.tag)

	target_tag := substring(verifier.input_tag, 0, 10)
}
...
raw_target_tag = target_tag {
	apply_target_filter_args
	not apply_target_filter_verifier
	target_tag := substring(args.input_tag, 0, 10)
}
