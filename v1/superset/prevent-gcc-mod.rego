...
violations = j {
    ...
    # Checks if gcc is in the path
    contains(modification.path, "gcc")
...
reason = v {
	Allow
	v := "No modification of gcc execuatable were found"
}

reason = v {
	not Allow
	count(violations) != 0
	v := sprintf("Possible modification of gcc executable found: %d", [count(violations)])
}
